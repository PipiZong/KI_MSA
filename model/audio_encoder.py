...
class TfEncoder(nn.Module):
    ...
    def forward(self, src, has_mask=True, src_key_padding_mask=None, output_hidden_states=False):
        ...
        if output_hidden_states==True:
            ...
            return output.transpose(0, 1), hidden_states  #transpose(0, 1)
        else:
...
class AudioEncoder(nn.Module):
    def __init__(self, name=None, fea_size=None, encoder_fea_dim=None, nhead=None, dim_feedforward=None,
        ...
        if nhead is None:
            nhead = config.MOSI.downStream.audio_nhead
        if drop_out is None:
            drop_out = config.MOSI.downStream.audio_drop_out
        if dim_feedforward is None:
            dim_feedforward = config.MOSI.downStream.encoder_fea_dim
        if num_layers is None:
            num_layers = config.MOSI.downStream.audio_tf_num_layers
    ...
    def forward(self, audio, key_padding_mask, device=None, output_hidden_states=False):
        if device is None:
            device = self.device
        if output_hidden_states == True:
            xx, hidden_states = self.encoder(audio, has_mask=False, src_key_padding_mask=key_padding_mask, output_hidden_states=output_hidden_states)
            x = self.layernorm(xx)
