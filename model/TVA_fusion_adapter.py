...
class TVA_fusion(nn.Module):
    ...
    def forward(self, sample1, return_loss=True, return_emb=False, device=None):
        ...
        ############
        x_t_embed, t_hidden_states, t_output = self.text_encoder(text1, device=device, output_hidden_states=True)
        ...
        hidden_states_last = self.pooler_t(hidden_states_last)
        x_t_embed = self.com_dense_t(torch.cat([x_t_embed, hidden_states_last],dim=1))
        x_t_embed = x_t_embed.squeeze()

        x_v_embed, v_hidden_states, v_output = self.vision_encoder(vision1, key_padding_mask=key_padding_mask_V1, device=device, output_hidden_states=True)
        ...
        hidden_states_last = torch.mean(hidden_states_last, dim=-2, keepdim=True)
        x_v_embed = self.com_dense_v(torch.cat([x_v_embed, hidden_states_last], dim=2))
        x_v_embed = x_v_embed.squeeze()

        x_a_embed, a_hidden_states, a_output = self.audio_encoder(audio1, key_padding_mask=key_padding_mask_A1, device=device, output_hidden_states=True)
        ...
        hidden_states_last = torch.mean(hidden_states_last, dim=-2, keepdim=True)
        x_a_embed = self.com_dense_a(torch.cat([x_a_embed, hidden_states_last], dim=2))
        x_a_embed = x_a_embed.squeeze()
        ...
        if return_loss:
            ...
            loss = pred_loss + self.config.MOSI.downStream.lamda * mono_task_loss
            if return_emb:
